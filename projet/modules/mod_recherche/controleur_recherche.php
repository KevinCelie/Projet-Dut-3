...
Class Controleur_Recherche{
    ...
    public function affiche_page_recherche() {
        if (isset($_POST['champRecherche'])) {
            $profil = $this -> modele -> effectuerRechercheProfil($_POST['champRecherche']);
            $projet = $this -> modele -> effectuerRechercheProjet($_POST['champRecherche']);
            $this -> vue -> affichePageRecherche($profil,$projet);
        }
